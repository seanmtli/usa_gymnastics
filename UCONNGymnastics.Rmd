...
```{r cutoffexample}
vaultcutoffs <- getmedalcutoffs(VT_ranks)
```
...
calculatePodiumPercentages <- function(usa, thresholds) {
  # Number of USA gymnasts
  num_USAgymnasts <- nrow(usa)
  
  # Initialize an empty dataframe for the results
  results <- data.frame(
    firstName = usa[, 1],
    lastName = usa[, 2],
    thirdPlace = numeric(num_USAgymnasts),
    secondPlace = numeric(num_USAgymnasts),
    firstPlace = numeric(num_USAgymnasts),
    stringsAsFactors = FALSE
  )
  
  # Iterating over each set of scores to give medals
  for (i in 1:1000) { ## THIS IS NOT A SIM
    # Extract the scores for the ith sample
    sample_scores <- df[, i + 2]
    
    # Rank the scores
    ranks <- rank(-sample_scores, ties.method = "min")
    
    # Update the count for first, second, and third places
    results$firstPlace <- results$firstPlace + (ranks == 1)
    results$secondPlace <- results$secondPlace + (ranks == 2)
    results$thirdPlace <- results$thirdPlace + (ranks == 3)
  }

  # Convert counts to percentages
  results$firstPlace <- (results$firstPlace / 1000) * 100
  results$secondPlace <- (results$secondPlace / 1000) * 100
  results$thirdPlace <- (results$thirdPlace / 1000) * 100

  # Sort the dataframe by firstPlace in descending order
  results <- results[order(-results$firstPlace), ]
