...
If our goal is to put forth the best lineup to maximize medal count, we explore who the top US candidates are in each apparatus and all-around. 
...
data_2022_2023$LastName[data_2022_2023$LastName == "SKINNER"] <- "Skinner"
```
...
topusmen <- rbind(top5usa("m", "HB"),top5usa("m","PB"), top5usa("m","FX"), top5usa("m","VT"), top5usa("m","PH"), top5usa("m","SR")) %>% group_by(Name) 
```
...
u5w <- ggplot(topusawomen, aes(y = reorder(Name,mean), x = Score, color = Name)) + 
  ...
  labs(title = "Top 5 USA gymnastics women based on mean score", subtitle = "Biles, Jones appear in top 5 across all 4 apparatus",
       y = " " )+ theme_bw()
u5w
```
Taking a glance at the top US women, we can see that Simone Biles by far and away is the best USA gymnast. That is quite unsurprising, given that she has the most gymnastics medals in the world and has continued to perform at an elite level in all events. We can almost lock her in to make the Paris 2024 roster. Shilese Jones is the only other gymnast to place in the top 5 US mean scores of each of the 4 apparatus, making her another top pick. 

```{r usmen, echo=FALSE}
u5m <- ggplot(topusmen, aes(y = reorder(Name,mean), x = Score, color = Name)) + 
  geom_point(alpha = 0.5) +
  facet_wrap(~Apparatus, drop = TRUE) + labs(title = "Top 5 USA gymnastics men based on mean score", subtitle = "Malone appears in top 5 across 4 out of 6 apparatuses",
       y = " ") + theme_bw() + theme(axis.text.y.left = element_text(size=8), legend.position = "none")
u5m
```
The US men's data is not as clear. There are a total of 13 gymnasts who place in the top 5 of at least one of the 6 events. However, we see that Brody Malone places in the top 5 for 4/6 events, and Asher Hong, Khoi Young, Yul Moldauer place in the top 5 for 3/6 events. 
